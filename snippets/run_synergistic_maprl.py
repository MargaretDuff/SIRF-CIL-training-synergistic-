...
G = fn.OperatorCompositionFunction(alpha* fn.SmoothMixedL21Norm(epsilon=1e-4), 
                                    op.CompositionOperator(MergeBlockDataContainers(joint_grad.range_geometry(),[1,5]), joint_grad))

maprl_synergistic = MAPRL(initial_estimate=initial_estimate, data_fidelity=F, prior=G,
                step_size=1, relaxation_eta=0.1, max_iteration=25, update_objective_interval=1)
